...
export const handleGetAllEvents = async (req: Request, res: Response) => {
  ...
  export const handleGetEventById = async (
    ...
    if (!event) return res.status(404).json({ data: "Request not found" });
  ...
  export const handleCreateEvent = async (req: Request, res: Response) => {
    ...
    if (!error) {
      const { name, agenda, description, posterUrl, startDate, endDate, theme, location } = req.body;
  ...
  export const handleUpdateEventById = async (
    ...
    for (const update of updates) {
      if (
        !allowedUpdateFields.includes(update as keyof Prisma.eventUpdateInput)
      )
